...
class AgentClient(private val vertx: Vertx, private val config: AppConfig) {
    ...
    suspend fun embed(request: EmbedRequest): EmbedResponse {
        logger.debug { "Embedding document: ${request.page_id} - ${request.title}" }
        ...
        if (response.statusCode() != 200) {
            throw RuntimeException(
                    "Failed to embed document: ${response.statusCode()} - ${response.bodyAsString()}"
            )
        }

        return json.decodeFromString<EmbedResponse>(response.bodyAsString())
    }
